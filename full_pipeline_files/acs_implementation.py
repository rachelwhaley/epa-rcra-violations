...
df_acs= get_acs_data(years, ('NAME','B19013_001E','B01003_001E','B19001_002E','B19001_003E', 'B19001_004E',
...
evalu_11 = smaller_data_time(evalu, 'EVALUATION_START_DATE', 2011) 
...
evalu_miss, evalu_nonmiss = keep_miss_nonmiss(df, ['LONGITUDE83','LATITUDE83'])
...
zip_to_zta = pd.read_excel('/Users/csolisu/Downloads/zip_to_zcta_2017.xlsx',
...
drop_features(entire_df, drop_eval)

for key, val in dcat.items():
    entire_df = unite_the_perc(key,entire_df,val)

for _, val in unique_cat.items():
    change_cat(entire_df,val)
...
zcta_file = '/Users/csolisu/Downloads/zip_to_zcta_2017.xlsx'

df_acs_2016 = df_acs[df_acs['year'] == 2016]
fac_miss, fac_nonmiss = keep_miss_nonmiss(fac, ['LONGITUDE83','LATITUDE83'])
...
zip_to_zta = pd.read_excel(zcta_file,
                            converters={'ZIP_CODE': '{:0>5}'.format,'ZCTA': '{:0>5}'.format })
...
drop_features(entire_fac, drop_fac)

for key, val in dcat.items():
    entire_fac = unite_the_perc(key,entire_fac,val)

for _, val in unique_cat.items():
    change_cat(entire_fac,val)
